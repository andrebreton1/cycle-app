...
function getFollicularDuration(cycleLen = cycleLength.value) {
  return cycleLen - FIXED_PHASE_DAYS
}

function getPhaseBoundaries(cycleLen = cycleLength.value) {
  const follicularDuration = getFollicularDuration(cycleLen)
...
async function saveCycleDuration(value) {
  ...
  if (cycles.value[0]) {
    cycles.value[0] = { ...cycles.value[0], duree_cycle: clamped }
  }
}

function onCycleLengthInput(event) {
  saveCycleDuration(event.target.value)
}

async function confirmPeriodStart() {
  saving.value = true
  const payload = { date_debut: todayStr, duree_cycle: CYCLE_DEFAULT }
  if (latestCycle.value?.user_id) {
    payload.user_id = latestCycle.value.user_id
  }
...
<template>
  <div class="app" :class="{ 'app--settings': showSettings }">
    ...
    <template v-if="!showSettings">
      ...
      <p v-else-if="latestCycle" class="status">
        Last period: {{ formatDateEn(latestCycle.date_debut) }}
      </p>
      ...
      <section class="calendar card" aria-label="Monthly calendar">
        <div class="month-nav">
          ...
          <button type="button" class="nav-btn" aria-label="Next month" @click="nextMonth">
            ›
          </button>
        </div>
        ...
        <div class="grid">
          <button
            ...
            :style="cell.phase && cell.inMonth ? {
              backgroundColor: cell.phase.color,
              color: cell.phase.textColor,
            } : undefined"
            :disabled="!cell.inMonth || !cell.phase"
            @click.stop="openDay(cell)"
          >
            <span class="day__num">{{ cell.day }}</span>
            <span v-if="cell.phase && cell.inMonth" class="day__emoji" aria-hidden="true">
              {{ cell.phase.emoji }}
            </span>
          </button>
        </div>
      ...
      <div
        ...
        @click.stop
      >
        <button type="button" class="popover__close" aria-label="Close" @click="closePopover">
          ×
        </button>
        <h3 id="popover-title" class="popover__title">{{ popover.phase.name }}</h3>
        <p class="popover__meta">
          {{ popover.dateStr }} — Day {{ popover.cycleDay }} of {{ cycleLength }} days
        </p>
    ...
    <template v-else>
      <header class="header card">
        <button type="button" class="icon-btn" aria-label="Back to calendar" @click="backToCalendar">
          ‹
        </button>
        ...
        <ul v-else class="history__list">
          <li v-for="entry in historyEntries" :key="entry.id" class="history__item">
            <div class="history__info">
              ...
              <span
                v-if="entry.observedDuration != null"
                class="history__duration"
              >
                {{ entry.observedDuration }} days
              </span>
    ...
    <div v-if="dialogMode" class="dialog-backdrop" @click.self="closeDialog">
      <div
        v-if="dialogMode === 'period'"
        class="dialog card"
        role="alertdialog"
        aria-labelledby="dlg-period-title"
        @click.stop
      >
        <h3 id="dlg-period-title">Are you sure?</h3>
        <p>Save today ({{ todayStr }}) as the start of your period?</p>
        <div class="dialog__actions">
          <button type="button" class="btn btn--ghost" :disabled="saving" @click="closeDialog">
            Cancel
          </button>
          <button type="button" class="btn btn--primary" :disabled="saving" @click="confirmPeriodStart">
      ...
      <div
        ...
        @click.stop
      >
        <h3 id="dlg-delete-title">Delete this entry?</h3>
        <p>This action cannot be undone.</p>
        <div class="dialog__actions">
          <button type="button" class="btn btn--ghost" :disabled="!!deletingId" @click="closeDialog">
            Cancel
          </button>
          <button type="button" class="btn btn--primary" :disabled="!!deletingId" @click="confirmDelete">
...
.day--today {
  box-shadow: inset 0 0 0 2px rgb(244 143 177 / 65%);
}
...
.cycle-range:disabled {
  opacity: 0.45;
  cursor: not-allowed;
}

.cycle-range::-webkit-slider-thumb {
  -webkit-appearance: none;
  width: 26px;
  height: 26px;
  border-radius: 50%;
  ...
  box-shadow: 0 2px 8px rgb(244 143 177 / 40%);
  cursor: pointer;
}

.cycle-range::-moz-range-thumb {
  width: 26px;
  height: 26px;
  border: 3px solid var(--accent);
  border-radius: 50%;
